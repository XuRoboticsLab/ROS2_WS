...
<div class="card">
  <div class="sec">已保存位姿</div>
  <div style="display:flex;gap:8px;margin-bottom:12px">
    <input type="text" id="pose-name" placeholder="输入位姿名称"
      style="flex:1;padding:10px 12px;border:1.5px solid var(--border);border-radius:8px;
             font-size:.9rem;outline:none;color:var(--text);background:var(--bg)">
...
function renderPoses(poses){
  ...
  if(!entries.length){
    el.innerHTML='<div style="color:var(--muted);font-size:.85rem;text-align:center;padding:8px 0">暂无保存的位姿</div>';
    return;
  }
  el.innerHTML=entries.map(([name])=>`
    <div style="display:flex;align-items:center;gap:8px;margin-bottom:8px">
      <div style="flex:1;padding:10px 12px;border:1.5px solid var(--border);border-radius:8px;
        ...
        ${_esc(name)}</div>
      <button data-name="${_esc(name)}" onclick="gotoPoseBtn(this)"
        style="padding:10px 14px;background:var(--accent);color:#fff;border:none;
...
async function savePose(){
  ...
  if(!name){setStatus('请输入位姿名称');return;}
  try{
    const r=await fetch('/poses',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify({name})});
    const d=await r.json();
    if(d.ok){document.getElementById('pose-name').value='';loadPoses();setStatus('已保存: '+name);}
...
async function gotoPoseBtn(btn){
  ...
  setStatus('前往: '+name);
  try{
    const r=await fetch('/actions/goto_pose/'+encodeURIComponent(name),{method:'POST'});
    const d=await r.json();
    if(!d.ok){setStatus('启动失败: '+(d.error||''));btn.disabled=false;return;}
    const poll=setInterval(async()=>{
      try{
        const s=await(await fetch('/action_status')).json();
        if(!s.running){clearInterval(poll);btn.disabled=false;setStatus('已到达: '+name+' '+new Date().toLocaleTimeString());}
...
def start(state, robot, port: int, arm_name: str, poses_file: str = ""):
    ...
    def save_pose():
        ...
        poses[name] = list(state.last_valid_joint_pos)
        _persist_poses()
        print(f"[ParamServer] 保存位姿 '{name}': {[f'{v:.3f}' for v in poses[name]]}")
        return jsonify({"ok": True})
    ...
    def action_goto_pose(name):
        ...
        action_running[0] = True
        threading.Thread(target=_run_goto_pose_thread, args=(list(poses[name]),),
